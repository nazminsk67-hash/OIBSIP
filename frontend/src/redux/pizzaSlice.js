...
const ADMIN_PIZZA_CACHE_MS = 30_000
...
export const fetchAdminPizzas = createAsyncThunk(
  ...
  },
  {
    condition: (_, { getState }) => {
      const { loading, adminFetchedAt } = getState().pizza
      if (loading) return false
      if (adminFetchedAt && Date.now() - adminFetchedAt < ADMIN_PIZZA_CACHE_MS) return false
      return true
    },
  }
)
