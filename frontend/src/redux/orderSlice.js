...
const ADMIN_ORDERS_CACHE_MS = 30_000
...
export const fetchAllOrders = createAsyncThunk(
  ...
  },
  {
    condition: (_, { getState }) => {
      const { loading, adminFetchedAt } = getState().order
      if (loading) return false
      if (adminFetchedAt && Date.now() - adminFetchedAt < ADMIN_ORDERS_CACHE_MS) return false
      return true
    },
  }
)
...
const orderSlice = createSlice({
  ...
  reducers: {
    ...
    updateOrderStatus: (state, action) => {
      const { orderId, status, statusHistory, lastStatusUpdateAt } = action.payload
      const idx = state.orders.findIndex(o => o._id === orderId)
      if (idx !== -1) {
        state.orders[idx].status = status
        if (statusHistory) state.orders[idx].statusHistory = statusHistory
        if (lastStatusUpdateAt) state.orders[idx].lastStatusUpdateAt = lastStatusUpdateAt
      }
      if (state.currentOrder?._id === orderId) {
        state.currentOrder.status = status
        if (statusHistory) state.currentOrder.statusHistory = statusHistory
        if (lastStatusUpdateAt) state.currentOrder.lastStatusUpdateAt = lastStatusUpdateAt
      }
    },
