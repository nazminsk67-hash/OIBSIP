...
export default function AdminDelivery() {
  ...
  useEffect(() => {
    fetchAssignments()
    // load recent admin orders for assignment dropdown
    const fetchOrders = async () => {
      try {
        ...
        console.warn('Failed to load orders for delivery assignment:', err.message || err)
      }
    }
    fetchOrders()
  }, [fetchAssignments])
                ...
                >
                  ...
                  {orders.map((o) => (
                    <option key={o._id} value={o._id}>
                      {`#${o._id.slice(-8).toUpperCase()} - ${o.user?.name || o.user?.email || 'Customer'}`}
                    </option>
                  ))}
                </select>
              </label>
