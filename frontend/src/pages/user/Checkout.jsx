...
import CouponRewardPanel from '../../components/checkout/CouponRewardPanel'
...
export default function Checkout() {
  ...
  const [rewardRedemption, setRewardRedemption] = useState({ pointsUsed: 0, rewardDiscount: 0 })

  const couponDiscount = coupon?.discount || 0
  const rewardDiscount = rewardRedemption.rewardDiscount || 0
  const finalTotal = Math.max(0, total - couponDiscount - rewardDiscount)
  ...
  const loadRewardSummary = async () => {
    try {
      ...
      console.warn('Reward summary load failed:', err)
    }
  }
  ...
  const handlePlaceOrder = async () => {
    ...
    const orderPayload = {
      ...
      rewardPointsUsed: rewardRedemption.pointsUsed || 0,
      ...
      if (paymentMethod === 'online') {
        ...
        const options = {
          ...
          handler: async (resp) => {
            try {
              const verifyPayload = {
                razorpayOrderId: resp.razorpay_order_id || data.orderId,
                razorpayPaymentId: resp.razorpay_payment_id,
                razorpaySignature: resp.razorpay_signature,
                amount: finalTotal,
              }

              const { data: verifyData } = await orderApi.verifyPayment(verifyPayload)
              ...
              await dispatch(
                placeOrder({
                  ...
                  razorpaySignature: verifyPayload.razorpaySignature,
                })
              ).unwrap()
              dispatch(clearItems())
              ...
              const msg = err.payload || err.message || 'Unable to complete order after payment.'
              setErrorMessage(msg)
              toast.error(msg)
            }
          },
  ...
  return (
    <div className="page-shell" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1.4fr_0.75fr]">
        <div className="space-y-6 rounded-[2rem] border p-6" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)', boxShadow: 'var(--shadow-sm)' }}>
          ...
          <CouponRewardPanel
            ...
            currentRedemption={rewardRedemption}
            onApplyCoupon={setCoupon}
            onClearCoupon={() => setCoupon(null)}
            onRedeemPoints={(redemption) => setRewardRedemption(redemption)}
          />
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-2xl p-4" style={{ backgroundColor: 'var(--bg-secondary)' }}>
              ...
              <div className="flex items-center justify-between">
                ...
                <p className="text-2xl font-bold" style={{ color: 'var(--accent-primary)' }}>{formatPrice(finalTotal)}</p>
              </div>
            </div>
