    ...
    <div className="page-shell max-w-6xl">
      ...
      <div className="space-y-8">
        {orders.map((order) => {
          ...
          return (
            <div key={order._id} className="min-w-0 rounded-[2rem] border border-slate-200 bg-white p-4 sm:p-6 shadow-sm">
              ...
              {order.lastStatusUpdateAt && (
                <p className="text-xs text-slate-500 mb-4">
                  Last updated: {formatDateTime(order.lastStatusUpdateAt)}
                </p>
              )}
                  ...
                  {progressSteps.map((step, idx) => {
                    ...
                    return (
                      <div key={step} className="flex min-w-0 flex-col items-center gap-2">
                        <div
                          className={`flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full text-xs font-bold transition ${
                            status === 'completed'
                        ...
                        <p className="text-center text-[10px] sm:text-xs font-medium leading-tight text-slate-700">{step}</p>
                      </div>
                    )
                  })}
                </div>
                ...
                <div className="mb-6 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  ...
                  <ol className="space-y-3">
                    {[...(order.statusHistory || [])].reverse().map((entry, i) => (
                      <li key={`${entry.status}-${entry.updatedAt}-${i}`} className="flex gap-3 text-sm">
