...
function PizzaCardInner({ pizza }) {
  ...
  return (
    <article
      className="group theme-panel w-full cursor-pointer overflow-hidden rounded-[2rem] shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl"
      onClick={() => navigate(`/pizza/${pizza._id}`)}
    >
      <div className="relative h-48 sm:h-56 overflow-hidden bg-slate-100">
        <ImageWithFallback src={imageSrc} alt={pizza.name} className="h-full w-full object-cover" />
      ...
      <div className="space-y-4 p-5">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
          ...
          <button
            ...
            className="btn-mobile-full shrink-0 rounded-full bg-primary-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-primary-700"
          >
            Order
          </button>
        </div>
