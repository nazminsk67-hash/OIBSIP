    ...
    if (orderData.pizzas && orderData.pizzas.length) {
      ...
      for (const p of orderData.pizzas) {
        ...
        if (Array.isArray(p.toppings)) {
          for (const t of p.toppings) {
            ...
            ingredientCount[t.ingredientId] = (ingredientCount[t.ingredientId] || 0) + qty
          }
        }
      ...
      if (pizzaIds.length) {
        ...
        for (const pizzaDoc of pizzasFromDb) {
          const qty = orderData.pizzas.find(p => p.pizza?.toString() === pizzaDoc._id.toString())?.quantity || 1
          for (const ingId of pizzaDoc.inventoryIngredients || []) {
            ingredientCount[ingId.toString()] = (ingredientCount[ingId.toString()] || 0) + qty
          }
        }
      }
